...
public:
	Iron(SDL_Rect dst_iron) : weight(0), ItemInterface(dst_iron, IRON) {
		SDL_Surface* iron_sheet_surface = IMG_Load("../../Resources/metal_motion.png"); // 이미지 파일을 가져옴
		SDL_SetColorKey(iron_sheet_surface, SDL_TRUE, SDL_MapRGB(iron_sheet_surface->format, 50, 60, 57));
		iron_sheet_texture = SDL_CreateTextureFromSurface(g_renderer, iron_sheet_surface);

		g_iron_source_rect.x = 0; // 사과 가져오기
		g_iron_source_rect.y = 0;
		g_iron_source_rect.w = 64;
		g_iron_source_rect.h = 64;
